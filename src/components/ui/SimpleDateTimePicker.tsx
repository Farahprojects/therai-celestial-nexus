...
export const SimpleDateTimePicker: React.FC<SimpleDateTimePickerProps> = ({
  ...
  const ampm = timeValue && timeValue.includes(':') ? parseInt(timeValue.split(':')[0]) >= 12 ? 'PM' : 'AM' : 'AM';
  ...
  const containerClass = (hasError: boolean) => cn("inline-flex items-center bg-white border-2 rounded-full transition-all duration-200 px-3 py-1", hasError ? "border-red-300" : "border-gray-200 focus-within:border-gray-400");
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <div className={containerClass(hasDateError)}>
            <input ref={dayRef} type="text" defaultValue={day} onChange={e => {
            const newValue = handleDateInput(e.target.value, 'day', monthRef);
            updateDate(newValue, month, year);
          }} className={cn(inputClass, "w-8")} placeholder="DD" maxLength={2} />
            <span className="text-gray-400 text-sm">/</span>
            <input ref={monthRef} type="text" defaultValue={month} onChange={e => {
            const newValue = handleDateInput(e.target.value, 'month', yearRef);
            updateDate(day, newValue, year);
          }} className={cn(inputClass, "w-8")} placeholder="MM" maxLength={2} />
            <span className="text-gray-400 text-sm">/</span>
            <input ref={yearRef} type="text" defaultValue={year} onChange={e => {
            const newValue = handleDateInput(e.target.value, 'year');
            updateDate(day, month, newValue);
          }} className={cn(inputClass, "w-12")} placeholder="YYYY" maxLength={4} />
          </div>
          ...
          <Popover open={isDateOpen} onOpenChange={setIsDateOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <SimpleDatePicker value={dateValue} onChange={onDateChange} onClose={() => setIsDateOpen(false)} hasError={hasDateError} />
            </PopoverContent>
          </Popover>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 justify-end">
        <label className="text-sm font-medium text-gray-700">Time *</label>
          <Popover open={isTimeOpen} onOpenChange={setIsTimeOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <SimpleTimePicker value={timeValue} onChange={onTimeChange} onClose={() => setIsTimeOpen(false)} hasError={hasTimeError} />
            </PopoverContent>
          </Popover>
          
          <div className={containerClass(hasTimeError)}>
            <input ref={hourRef} type="text" defaultValue={displayHour} onChange={e => {
            const newValue = handleTimeInput(e.target.value, 'hour', minuteRef);
            updateTime(newValue, minute, ampm);
          }} className={cn(inputClass, "w-8")} placeholder="HH" maxLength={2} />
            <span className="text-gray-400 text-sm">:</span>
            <input ref={minuteRef} type="text" defaultValue={displayMinute} onChange={e => {
