  ...
  return <div className="min-h-screen flex flex-col bg-white">
      ...
      <AnimatePresence>
        {isMenuOpen && <>
          ...
          duration: 0.2
        }} className="fixed inset-0 z-30 bg-black/50" onClick={() => setIsMenuOpen(false)} />
          ...
          stiffness: 200
              ...
              <div className="p-5 pt-20">
                {/* Close button */}
                <button onClick={() => setIsMenuOpen(false)} className="absolute top-6 right-5 text-gray-700 hover:text-gray-900 transition-colors">
                  <X className="h-6 w-6" />
                </button>
                ...
                <div className="space-y-2">
                  ...
                  <Link to="/blog" className="block text-gray-700 hover:text-gray-900 transition-colors py-3 text-lg font-light" onClick={() => setIsMenuOpen(false)}>
                    Blog
                  </Link>
                  <Link to="/contact" className="block text-gray-700 hover:text-gray-900 transition-colors py-3 text-lg font-light" onClick={() => setIsMenuOpen(false)}>
                    Contact
                  </Link>
                  <Link to="/legal" className="block text-gray-700 hover:text-gray-900 transition-colors py-3 text-lg font-light" onClick={() => setIsMenuOpen(false)}>
                    Legal
                  </Link>
                </div>
              </div>
      ...
      <section className="mt-auto bg-black text-white px-5 pt-8 pb-10 rounded-t-3xl">
        <div className="space-y-3">
          {isNativeApp ? <CapacitorSocialLogin onSuccess={handleSocialLoginSuccess} /> : <>
              <Button type="button" className="w-full h-12 rounded-full bg-white text-black hover:bg-white/90" onClick={async () => {
          ...
          <Button type="button" className="w-full h-12 rounded-full bg-white/0 text-white border border-white hover:bg-white/10" onClick={() => openAuthModal('login')}>
            Log in
          </Button>
        </div>
      </section>
